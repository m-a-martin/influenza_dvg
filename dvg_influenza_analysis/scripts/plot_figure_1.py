...
def run():
	...
	plasmid_dat = {i: 
		pd.read_csv(path, sep='\t') for i,path in id_dir_dict.items() if i in plasmid_dict.values()}
	for key, value in plasmid_dat.items():
		value['run'] = key
	...
	segment_read_support_per_dvg = {idx: 
		i['Total_support'] for idx, i in all_plasmid_dat.groupby('segment')}
	...
	axs[0].set_ylabel('DVGs per plasmid sequence', size=mpl.rcParams['axes.titlesize'])
	...
	axs[1].set_ylabel('supporting reads per DVG', size=mpl.rcParams['axes.titlesize'])
