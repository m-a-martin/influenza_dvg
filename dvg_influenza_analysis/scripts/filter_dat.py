...
def run():
	...
	# filter data
	filtered_dat = \
		filter_dvg_dat(dat, min_reads=args.minReads, 
	...
	if args.segments:
		filtered_dat.to_csv('.'.join(args.dat.split('.')[:-1])+ \
			f'_{args.minReads}_{args.only_shared}_{args.filter_plasmid}_{args.minDel}_{"_".join(args.segments)}.tsv',
			sep='\t', index=None)
	else:
		filtered_dat.to_csv('.'.join(args.dat.split('.')[:-1])+ \
			f'_{args.minReads}_{args.only_shared}_{args.filter_plasmid}_{args.minDel}_all.tsv',
			sep='\t', index=None)
