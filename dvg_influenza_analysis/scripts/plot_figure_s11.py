...
import string
try: 
	from utils import plot_style,  find_premature_stop, import_seqs
except:
	from scripts.utils import plot_style, find_premature_stop, import_seqs
...
def run():
	...
	dat['premature_stop_x'] = dat.apply(lambda k: find_premature_stop(k.Start - args.padSize, k.Stop-args.padSize, 
		cds_dict[k.Segment], ref_dict[k.Segment + '_' + k.Run_x]), axis=1)

	dat['premature_stop_y'] = dat.apply(
		lambda k: 
			find_premature_stop(k.Start - args.padSize, k.Stop-args.padSize, 
				cds_dict[k.Segment], ref_dict[k.Segment + '_' + k.Run_x]) if type(k.Run_y) == str else np.nan, axis=1)

	dat['premature_stop'] = \
		(dat['premature_stop_x'] | np.where(~dat['premature_stop_y'].isnull(), dat['premature_stop_y'], False))
	...
	with open('figures/final/figure_s11.txt', 'w') as fp:
		for line in output:
			fp.write(line + '\n')
	...
	for idx, seg in enumerate(segments):
		...
		axs[0,idx].xaxis.set_major_locator(tck.MultipleLocator(3))
		axs[0,idx].yaxis.set_major_locator(tck.MultipleLocator(3))
	...
	x_pos = [-0.22, -0.16, -0.16, -0.09, -0.075, -0.075]
	...
	fig.suptitle('Figure S11')
	fig.savefig('figures/final/figure_s11.pdf')
	plt.close()
